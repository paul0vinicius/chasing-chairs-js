...
export const GameOverScreen: FC<GameOverScreenProps> = ({ onReturnToMenu, players }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center w-full h-[100dvh] bg-black/60 animate-fade-in px-4 px-safe-left px-safe-right pt-safe-top pb-safe-bottom">
      <RotateOverlay />

      <div className="w-full max-w-md max-h-[85dvh] overflow-y-auto bg-black/80 border-4 border-white p-6 md:p-8 shadow-[8px_8px_0px_rgba(0,0,0,1)] flex flex-col items-center text-center">
        {/* Título Principal */}
        ...
        {/* Lista de Pontuações (Placar Final) com margem interna (padding) */}
        <div className="w-full p-4 mb-8 bg-black/40 border-2 border-white/10 rounded-md shadow-inner">
          <div className="flex flex-col w-full gap-3 max-h-[30vh] overflow-y-auto pr-2">
            {sortedPlayers.map((player, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-pixel text-xl md:text-2xl whitespace-nowrap">
                    {player.score} pts
                  </span>
                </div>
              )
